...
export default function SurveyPageComponent({ id }) {
  ...
  // Fetch survey data from API based on id
  useEffect(() => {
    if (id) {
      const url = `https://localhost:7216/api/surveys/json/${id}`;
      ...
      fetch(url)
        ...
        .catch((error) => {
          ...
          setSurveyData(defaultSurveyData); // Use default data if fetch fails
